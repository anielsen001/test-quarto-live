...
```{pyodide}
import micropip
await micropip.install('inu')
await micropip.install('r3f')
...
er = rpy_t - hrpy_t
#plt.plot(t/60, er, label="attitude errors (rad)")

#plt.plot(t/60, [rpy_t[0], hrpy_t[0]], label="true and est roll (rad)")
#plt.plot(t/60, [rpy_t[1], hrpy_t[1]], label="true and est pitch (rad)")
#plt.plot(t/60, [rpy_t[2], hrpy_t[2]], label="true and est yaw (rad)")

```


```{pyodide}
#| label: fig-compare-truth-and-estimate
# Show the comparison of true path and estimate.
plt.figure()
plt.plot(yc/1e3, xc/1e3)
plt.plot(hyc/1e3, hxc/1e3)
plt.show()
```

```{pyodide}
#| label: fig-show-errors
plt.figure()
# Show the errors.
er = np.array([xc - hxc, yc - hyc, zc - hzc])
plt.plot(t/60, er.T, label="curvilinear errors (m)")
...
```{pyodide}
#| label: fig-show-attitude-errors
plt.figure()
er = rpy_t - hrpy_t
plt.plot(t/60, er.T, label="attitude errors (rad)")
plt.show()
```

```{pyodide}
#| label: fig-show-attitude-diffs-roll
plt.figure()
plt.plot(t/60,rpy_t[0],label="roll truth")
plt.plot(t/60,hrpy_t[0], label="roll estimate")
plt.legend()
plt.show()
```

```{pyodide}
#| label: fig-show-attitude-diffs-pitch
plt.figure()
plt.plot(t/60,rpy_t[1],label="pitch truth")
plt.plot(t/60,hrpy_t[1], label="pitch estimate")
plt.legend()
plt.show()
```

```{pyodide}
#| label: fig-show-attitude-diffs-yaw
plt.figure()
plt.plot(t/60,rpy_t[2],label="yaw truth")
plt.plot(t/60,hrpy_t[2], label="yaw estimate")
plt.legend()
plt.show()
```
